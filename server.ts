...
Reply ONLY with a number representing the estimated minutes. Do not include any text, punctuation, or explanation. For example: 45`;
  ...
  app.post('/api/review-plan', requireAuth, async (req, res) => {
    ...
    try {
      const ai = new GoogleGenAI({ apiKey: geminiApiKey });
      const prompt = `Ти — досвідчений менеджер проектів і маркетолог. Говориш коротко, по суті, без води. Іноді можеш іронізувати або пожартувати — але лише якщо це доречно. Не розписуй очевидне.

Твоє завдання: проаналізувати задачу та доповнити її.

Задача: ${title}
Опис: ${description || 'Не вказано'}
Існуючі підзадачі:
${subtasks && subtasks.length > 0 ? subtasks.map((s: any) => '- ' + s.title).join('\n') : 'Жодної підзадачі — класика'}

Що треба зробити:
1. Напиши чіткий опис задачі (1–3 речення). Якщо опис вже є — покращ його або залиш як є.
2. Знайди пропущені кроки (погодження, аналітика, тести, дедлайни тощо). Не дублюй те, що вже є.
3. Дай коментар одним-двома реченнями — що не так або що добре. Можеш трохи з гумором, але по суті.
4. Оціни складність у Story Points (1–5):
   - 1 SP: до 1 год, рутина
   - 2 SP: 1–3 год, знайомий процес
   - 3 SP: до 1 дня, потрібен фокус
   - 4 SP: 2–3 дні, залежності, складно
   - 5 SP: тиждень+, стратегічний епік

Поверни ТІЛЬКИ чистий JSON, без markdown, без коментарів:
{
  "description": "Стислий опис задачі українською.",
  "explanation": "Короткий коментар. Що пропущено або що добре.",
  "newSubtasks": ["Нова підзадача (результат - ...)"],
  "storyPoints": 3
}`;
